...
impl<'a> LinuxIconSet<'a> {
    ...
    ///
    /// The document root must be an `<svg>` element in the SVG namespace, which
    /// keeps arbitrary XML from being stored as an icon.
    pub fn set_svg(&mut self, svg: impl Into<String>) -> Result<(), IconError> {
        const SVG_NS: &str = "http://www.w3.org/2000/svg";

        let svg = svg.into();
        // Parsed rather than string-matched so a prolog, comments or a doctype
        // ahead of the root element don't trip the check. DTDs are allowed to
        // match what `usvg` itself accepts.
        let opts = usvg::roxmltree::ParsingOptions {
            allow_dtd: true,
            ..Default::default()
        };
        let doc = usvg::roxmltree::Document::parse_with_options(&svg, opts)
            .map_err(|e| IconError::IconImage(format!("invalid SVG: {e}")))?;
        let root = doc.root_element().tag_name();
        if root.name() != "svg" || root.namespace() != Some(SVG_NS) {
            return Err(IconError::IconImage(
                "SVG must have an <svg> root element in the SVG namespace".to_string(),
            ));
        }
...
impl<'a> From<&'a crate::api::IconSet> for LinuxIconSet<'a> {
    fn from(icon_set: &'a crate::api::IconSet) -> Self {
        ...
        if let Some(svg) = &icon_set.svg {
            ...
            if let Err(e) = set.set_svg(svg.clone()) {
                log::warn!("dropping SVG variant during icon set import: {e}");
            }
        }
        set
    }
...
mod tests {
    ...
    /// Theme icons on disk (e.g. Adwaita) start with an XML declaration.
    const SVG_WITH_PROLOG: &str = concat!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n",
        "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"1\" height=\"1\"/>"
    );

    #[test]
    fn set_svg_accepts_markup_with_xml_prolog() {
        let mut set = LinuxIconSet::new();
        set.set_svg(SVG_WITH_PROLOG).unwrap();
        assert!(set.svg().is_some());
    }

    #[test]
    fn set_svg_accepts_markup_with_comment_and_doctype() {
        let svg = concat!(
            "<?xml version=\"1.0\"?>\n",
            "<!-- Generated by a theme -->\n",
            "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.1//EN\" ",
            "\"http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\">\n",
            "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"1\" height=\"1\"/>"
        );
        let mut set = LinuxIconSet::new();
        set.set_svg(svg).unwrap();
        assert!(set.svg().is_some());
    }

    #[test]
    fn set_svg_accepts_namespace_prefixed_root() {
        let mut set = LinuxIconSet::new();
        set.set_svg("<s:svg xmlns:s=\"http://www.w3.org/2000/svg\" width=\"1\" height=\"1\"/>")
            .unwrap();
        assert!(set.svg().is_some());
    }

    #[test]
    fn iconset_roundtrip_preserves_svg_with_prolog() {
        let mut set = LinuxIconSet::new();
        set.set_svg(SVG_WITH_PROLOG).unwrap();
        ...
        assert!(
            back.svg().is_some(),
            "SVG must survive re-validation on import"
        );
    }
}
